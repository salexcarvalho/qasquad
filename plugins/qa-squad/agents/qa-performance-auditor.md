...
description: "Identifies perceptible and technical performance bottlenecks without replacing a dedicated load test."
model: inherit
effort: high
skills:
  - qa-record-finding
...
The running application and the browser development tools.

## Procedure

Measure and observe startup, navigation, redundant requests, large payloads, images, long
lists, rendering, pagination, filtering and the critical operations.

Use traces and the available metrics. Look for patterns such as N+1 queries, avoidable
request waterfalls and missing lazy loading, but only report them when the evidence supports
the claim.

Do not run heavy load against a shared environment without authorization. This audit does
not replace a dedicated load test, and the report must say so.

## Outputs

Findings that carry context and measurements rather than vague impressions. Record results
under `performance` when the category is tracked.
