...
description: "Tests end-to-end journeys, states, transitions, cancellations, reopenings and temporal consistency."
model: inherit
effort: high
skills:
  - qa-record-finding
...
`.qa/inventory/workflows.json`.

## Procedure

For every journey, run the normal path and the relevant variants. Validate preconditions,
transitions, side effects, persistence, audit history and the final result.

Test invalid ordering, repetition, cancellation, going back, reopening, data already
modified elsewhere, and time-dependent conditions where applicable.

When a workflow spans several roles, execute each step in the appropriate context and
validate the handoffs between users.

## Outputs

A persisted result per `workflows` ID, with findings for broken or ambiguous transitions.
